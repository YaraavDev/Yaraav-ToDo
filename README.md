...
## ۷) توسعه بیشتر (پیشنهادی)

- افزودن Notification زمان‌بندی‌شده واقعی برای سررسید تسک‌ها (در حال حاضر نوتیفیکیشن دستی/پومودورو پیاده‌سازی شده؛ برای یادآوری دقیق سررسید، یک زمان‌بند در main process بر پایه `setTimeout`/`node-schedule` اضافه کنید).
- افزودن drag & drop واقعی چندسطحی برای Subtaskها.
- اتصال i18next در صورت نیاز به pluralization پیشرفته‌تر (فعلاً یک لایه i18n سبک و کافی برای این پروژه پیاده شده است).

---

ساخته‌شده با 🖤 برای **Yaraav To Do**
